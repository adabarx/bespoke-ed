...
use crate::{primatives::{Char, Layout, LayoutType, Line, Mother, Span, Text, TryMother}, InputRW, TokioRW};

#[async_trait]
trait Zipper {
    async fn prev(self) -> Option<OldZipper>;
    ...
    async fn replace_focus(self, new_node: Node) -> OldZipper;
}
...
impl MoveResult {
    ...
    pub fn inner_mut(&mut self) -> &mut OldZipper {
        match self {
            MoveResult::Moved(zip) => zip,
            MoveResult::DidntMove(zip) => zip,
        }
...
impl Node {
    pub fn get_layout(&self) -> Option<InputRW<Layout>> {
...
pub struct RootZipper {
    ...
    children: Vec<TokioRW<Layout>>
}

enum LayoutPrev {
    Root(RootZipper),
    Layout(Box<LayoutZipper>)
}

pub struct LayoutZipper {
    previous: LayoutPrev,
    focus: TokioRW<Layout>,
    children: Vec<TokioRW<Layout>>,
    left: Vec<TokioRW<Layout>>,
    right: Vec<TokioRW<Layout>>,
}

struct LineZipper {
    previous: LayoutZipper,
    focus: TokioRW<Line>,
    left: Vec<TokioRW<Line>>,
    right: Vec<TokioRW<Line>>,
    children: Vec<TokioRW<Span>>,
}

struct SpanZipper {
    previous: LineZipper,
    focus: TokioRW<Span>,
    left: Vec<TokioRW<Span>>,
    right: Vec<TokioRW<Span>>,
    children: Vec<TokioRW<Char>>,
}

struct CharZipper {
    previous: SpanZipper,
    focus: TokioRW<Char>,
    left: Vec<TokioRW<Char>>,
    right: Vec<TokioRW<Char>>,
}

#[derive(Clone)]
pub struct OldZipper {
...
impl OldZipper {
    ...
    pub async fn move_to_prev(mut self) -> Option<OldZipper> {
        self.focus.no_highlight();
    ...
    pub async fn move_left_catch_ignore(self) -> OldZipper {
        self.try_move_right().await.unwrap()
    }

    pub async fn move_right_catch_ignore(self) -> OldZipper {
        self.try_move_right().await.unwrap()
    }
    ...
    pub async fn add_child(mut self, node: Node, index: usize) -> OldZipper {
        let len = self.children.len();
        if index >= len {
    ...
    pub async fn replace_focus(mut self, new_node: Node) -> OldZipper {
...
impl Zipper for OldZipper {
    ...
    async fn daughter(self, mut index: usize) -> MoveResult {
        ...
        MoveResult::Moved(OldZipper { previous, focus, children, left, right })
    }
    ...
    async fn left_sister(self) -> MoveResult {
        ...
        MoveResult::Moved(OldZipper { focus, previous, children, left, right })
    }
    ...
    async fn replace_focus(mut self, new_node: Node) -> OldZipper {
